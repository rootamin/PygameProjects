...
class AllSprites(pygame.sprite.Group):
    def __init__(self):
        ...
        # dimensions
        self.sky_width = self.bg_sky.get_width() # bg and fg have the same width
        self.padding = WINDOW_WIDTH / 2 # 136 3:20
    ...
    def custom_draw(self, player):
        ...
        for x in range(self.sky_num):
            x_pos = -self.padding + (x * self.sky_width) # makes the map blit at the padding
...
class Main:
    ...
    def setup(self): # we call it under init
        ...
        # collision tiles
        for x, y, surf in tmx_map.get_layer_by_name('Level').tiles():  # this one needs collision mechanics and we are not passing it under the below for loop
            CollisionTile((x * 64, y * 64), surf, [self.all_sprites, self.collision_sprites])  # multiply by 64 is because of the eath tile in this project is 64 pixels

        # tiles
        for layer in ['BG', "BG Detail", "FG Detail Bottom", "FG Detail Top"]: # doesnt need any collision mechanics just overlapping for making it look real
            for x, y, surf in tmx_map.get_layer_by_name(layer).tiles():
                Tile((x * 64, y * 64), surf, self.all_sprites, LAYERS[layer])  # last attributes returns to z in the tile.py and because the dictionaries key values are the same it would work.
            ...
            for border in self.platform_border_rects:
                if platform.rect.colliderect(border):
                    if platform.direction.y < 0: # up
                        ...
                        platform.direction.y = 1 # platform moves downwards
                    else: # down
                        platform.rect.bottom = border.top
                        platform.pos.y = platform.rect.y
                        platform.direction.y = -1
            if platform.rect.colliderect(self.player.rect) and self.player.rect.centery > platform.rect.centery: # this makes the platform go up as soon as it hits the players head( and it would prevent the players head from sticking to bottom of the platform)
