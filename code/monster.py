...
class Monster:
    ...
    def face_player(self):
        ...
        if distance < self.notice_radius:
            if -0.5 < direction.y < 0.5:
                ...
                if direction.y < 0: # player to top
                    self.status = 'up_idle'
    ...
    def walk_to_player(self):
        ...
        else:  # if the player is not in the radius:
            self.direction = vector() # the player stops moving
...
class Coffin(Entity, Monster):
    ...
    def attack(self):
        ...
        if distance < self.attack_radius + 1 and not self.attacking:  # + 1 is added by myself because of the 0,49 = 50 and this checks -50 so =)
            self.attacking = True
            self.frame_index = 0
...
class Cactus(Entity, Monster):
    ...
    def attack(self):
        distance = self.get_player_distance_direction()[0]
        if distance < self.attack_radius + 1 and not self.attacking:
            self.attacking = True
            self.frame_index = 0
            self.bullet_shot = False
    ...
    def animate(self, dt):
        ...
        if int(self.frame_index) == 6 and self.attacking and not self.bullet_shot:  # OMG !
